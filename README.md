...
- **模拟模式 (`--mock`)**：无需真实网关也能生成演示数据，用于测试和报表展示
- **外部访问支持 (`--host`)**：默认绑定 `0.0.0.0`，支持从外部浏览器访问报表
- **快速演示**：使用 `Lance Memory测试用例集_精简3条.csv` 快速体验

## 🚀 快速演示

```bash
python3 -m eval.run \
    --cases "Lance Memory测试用例集_精简3条.csv" \
    --iteration-tag "demo-$(date +%Y%m%d)" \
    --mock \
    --serve
```
